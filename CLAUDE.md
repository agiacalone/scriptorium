...
- `node exam-reading-list-cli.js --exam-name "Midterm 1" --slug midterm_1 --course "CECS 326" --term sp26 --mains a.md,b.md --out ./out` — **exam reading-list** sub-tool: consolidates several topic `_lecture_main.md` files into one per-exam cue→source study guide (`<slug>_reading_list.md`). Driven by lectern's `reg-exam-readinglist`. Accepts `--textbook`, `--citation-key`, `--note`, `--note-title` overrides. `--format reference` emits the answer-free two-column **reference sheet** instead (`generators/exam-reference-sheet.js`): each Cornell statement with its blank left empty, grouped by section under a linked *Read* line, key terms grouped by defining source; it never reads `[answer::]`. Pair with `--readings-json` and `--retired-citation`.
