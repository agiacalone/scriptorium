...
// Decide whether the pdf-ua stage blocks the build. Untagged PDFs ALWAYS block
// (no accessibility tree). veraPDF PDF/UA-1 non-compliance blocks ONLY under
// --strict-a11y; by default it's advisory. Pure → unit-tested.
function evaluatePdfUaGate(stage, { strict } = {}) {
  const blocking = [];
  for (const r of stage.rows || []) {
    if (!r.pass) {
      blocking.push({ name: r.name, reason: `untagged — ${r.detail}` });
    } else if (strict && r.ua1 && !r.ua1.compliant) {
      blocking.push({ name: r.name, reason: r.ua1.detail });
    }
  }
