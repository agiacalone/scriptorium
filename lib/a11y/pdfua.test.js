...
import { interpretPdfinfo, interpretVeraJson, auditPdfUA, evaluatePdfUaGate } from './pdfua.js';
...
describe('evaluatePdfUaGate (blocking decision)', () => {
  const stage = (rows) => ({ stage: 'pdf-ua', ok: rows.every((r) => r.pass), rows });

  test('untagged PDF blocks regardless of --strict-a11y', () => {
    const s = stage([{ name: 'a.pdf', pass: false, detail: 'not tagged' }]);
    expect(evaluatePdfUaGate(s, { strict: false }).ok).toBe(false);
    expect(evaluatePdfUaGate(s, { strict: true }).ok).toBe(false);
    expect(evaluatePdfUaGate(s, { strict: false }).blocking[0].reason).toMatch(/untagged/i);
  });

  test('tagged-but-PDF/UA-non-compliant does NOT block by default', () => {
    const s = stage([{ name: 'a.pdf', pass: true, detail: 'tagged', ua1: { compliant: false, detail: '5 rules' } }]);
    expect(evaluatePdfUaGate(s, { strict: false }).ok).toBe(true);
    expect(evaluatePdfUaGate(s, { strict: false }).blocking).toEqual([]);
  });

  test('tagged-but-non-compliant BLOCKS under --strict-a11y', () => {
    const s = stage([{ name: 'a.pdf', pass: true, detail: 'tagged', ua1: { compliant: false, detail: 'PDF/UA-1 non-compliant — 5 rules' } }]);
    const r = evaluatePdfUaGate(s, { strict: true });
    expect(r.ok).toBe(false);
    expect(r.blocking[0].name).toBe('a.pdf');
    expect(r.blocking[0].reason).toMatch(/PDF\/UA-1/);
  });

  test('fully compliant passes under strict', () => {
    const s = stage([{ name: 'a.pdf', pass: true, detail: 'tagged', ua1: { compliant: true, detail: 'ok' } }]);
    expect(evaluatePdfUaGate(s, { strict: true }).ok).toBe(true);
  });

  test('empty stage is ok', () => {
    expect(evaluatePdfUaGate(stage([]), { strict: true }).ok).toBe(true);
  });
});
