...
function texPreamble(headerLeft, headerRight, opts = {}) {
  ...
  // PDF/UA-1 needs a document title in XMP (dc:title) — derive it from the
  // header title; strip TeX-special chars that would break the pdfstring.
  ...
  // MUST be the very first line, before \documentclass. Needs a tagging-capable TeX Live
  // (≥2024; verified on TL2026): produces a real StructTreeRoot under pdflatex. The
  // `table` testphase module is what lets `\tagpdfsetup{table/header-rows=…}` emit /TH
  // header cells (verified TH vs TD via pikepdf on TL2026). `pdfstandard=ua-1` writes the
  // PDF/UA-1 identifier into XMP and `pdfversion=1.7` forces the PDF-1.7 header PDF/UA-1
  // requires (default is 2.0) — both verified veraPDF-clean on TL2026.
...
function texComparisonTable(headers, rows) {
  ...
  const dataRows = rows
    ...
    .join("\n");
  // Group-scoped header-rows tagging: marks row 1's cells as /TH (ADA Title II,
  // issue #7). The surrounding {…} keeps the directive local so non-header layout
  // tables elsewhere keep their cells as /TD (verified no-leak via pikepdf).
  return `
\\vspace{4pt}
